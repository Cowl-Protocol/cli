...
| 🟡 | 2026-08-01 | Relayer daemon — adversarial cases against the real process over a stub chain, plus the mutants that prove each case bites | `src/relayer/{server,client,rebalance}.ts` | `90836c5` | 6 findings, all fixed: 1 Medium (one dust pool set the fee for a whole token), 2 Low (an endpoint hiccup killed the daemon; 6 uncapped RPC calls per anonymous request), 3 Informational. 8/8 mutants caught. One residual named | [relayer/](relayer/README.md) |
